...
DISTANCE_VECTOR = np.linspace(45, 75, num=2160)  # Suponiendo un ancho de imagen de 3840*2160 píxeles
...
vehicle_positions2 = {}
plate_speed = []
global lane1_mask_gray, lane2_mask_gray
lane1_mask_gray = None
lane2_mask_gray  = None
...
def convert_video_to_blob(gif_path):
...
def Hilo1():
    global terminate_flag1, plate_speed
    ...
    while cap.isOpened():
        ...
        for res in results_vehicle:
            ...
            for xmin, ymin, xmax, ymax, track_id in tracks:
                ...
                vehicle_positions2[track_id] = (xmin/6, ymin/4.5, xmax/6, ymax/4.5)

                x = int((xmin/6 + xmax/6) // 2)
                y = int((ymin/4.5 + ymax/4.5) // 2)
                with lock:
                    if lane1_mask_gray is not None and lane2_mask_gray is not None:
                        if 0 <= x < lane1_mask_gray.shape[1] and 0 <= y < lane1_mask_gray.shape[0]:
                            # Obtener el valor de la máscara en la posición del vehículo
                            lane1_value = lane1_mask_gray[y, x]
                            lane2_value = lane2_mask_gray[y, x]
                            #print(f"x: {x}, y: {y}, lane1_value: {lane1_value}, lane2_value: {lane2_value}")
                            #print (f"lane1: {lane1_value}, lane2: {lane2_value}")
                            if lane1_value > lane2_value:
                                vehicle_lanes[track_id] = "left"
                                print("left")
                            elif lane2_value > lane1_value:
                                vehicle_lanes[track_id] = "right"
                                print("right")
                            else:
                                vehicle_lanes[track_id] = "unknown"
                                print("unknown")
                        else:
                            vehicle_lanes[track_id] = "out_of_bounds"
                            print("unknowqqqn")

                cv2.putText(frame, f"ID: {track_id}", (xmin, ymin - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 0), 2)
        ...
        # Imprimir la asociación de track_id con carril
        for vid, lane in vehicle_lanes.items():
            print(f"Track ID: {vid} está en el carril: {lane}")
                ...
                for plate_box in plate_boxes:
                    ...
                    for vehicle_id, (v_xmin, v_ymin, v_xmax, v_ymax) in prev_boxes.items():
                        ...
                        if p_xmin >= v_xmin and p_ymin >= v_ymin and p_xmax <= v_xmax and p_ymax <= v_ymax:
                            ...
                            # Almacenar la velocidad en el diccionario
                            if vehicle_id not in vehicle_speeds:
                                vehicle_speeds[vehicle_id] = []
                            vehicle_speeds[vehicle_id].append(plate_speed)
                            cv2.putText(frame, f"{vehicle_speeds[vehicle_id][-1]:.2f} km/h", (p_xmin, p_ymin - 10), cv2.FONT_HERSHEY_SIMPLEX, 2.9, (255, 255, 255), 15)

                            """ if vehicle_speeds[vehicle_id][-1] > 50 and vehicle_lanes.get(track_id) == "left" and len(image_buffer)>3:
                                print(vehicle_id)
                                ...
                                try:
                                    ...
                                    if connection.is_connected():
                                        ...
                                        insert_query = "INSERT INTO registro (placa, velocidad, video,infraccion) VALUES (%s, %s, %s, %s)"
                                        record = (plate, f"{vehicle_speeds[vehicle_id][-1]:.2f}", gif_blob, 1)

                                        # Debug prints to check types
                                        print(f"plate: {plate}, type: {type(plate)}")
                                        print(f"average_speed: {str(vehicle_speeds[vehicle_id][-1])}, type: {type(str(vehicle_speeds[vehicle_id][-1]))}")
                                    ...
                                    if connection.is_connected():
                                        try:
                                            ...
                                            print("Error al cerrar el cursor:", e)
                                        connection.close()
    ...
    # Calcular el promedio de velocidad para cada vehicle_id
    average_speeds = {vid: sum(speeds)/len(speeds) for vid, speeds in vehicle_speeds.items()}
    

    # Imprimir las velocidades promedio
    for vid, avg_speed in average_speeds.items():
        print(f"Vehicle ID: {vid}, Average Speed: {avg_speed:.2f} km/h")


    # Evaluar condiciones de infracción
    for vehicle_id, avg_speed in average_speeds.items():
        lane = vehicle_lanes.get(vehicle_id, "unknown")
        plate = vehicle_plates.get(vehicle_id, "UNKNOWN")

        if avg_speed > 50 and lane == "left" and len(vehicle_speeds[vehicle_id]) > 5:
            gif_path = f'alert_{vehicle_id}.gif'
            video_blob = convert_video_to_blob(gif_path)
            infraction = 1
            ...
            try:
                ...
                cursor = connection.cursor()
                sql = """
                    INSERT INTO registro (placa, velocidad, video, infraccion)
                    VALUES (%s, %s, %s, %s)
                """
                cursor.execute(sql, (plate, avg_speed, video_blob, infraction))
                connection.commit()
                print(f"Infracción registrada para vehículo {vehicle_id}")
...
def Hilo2():
    ...
    while cap.isOpened():
        ...
        for result in results.xyxy[0]:
            ...
            if track_id is not None:
                ...
                with lock:
                    if len(text_easyocr) == 6 and pattern.match(text_easyocr):
                ...
                contains_similar_chars ='Q' in text_easyocr or 'O' in text_easyocr
                box_color = (0, 255, 0)
    ...
    # Imprimir la lista de track_ids y sus placas
    print("Lista de track_ids y sus placas:")
    for track_id, plate in vehicle_plates.items():
        print(f"Track ID: {track_id}, Placa: {plate}")
...
def Hilo3():
    ...
    for i in range(repetitions):
        ...
        while True:
            ...
            #frame = cv2.resize(image, (3840,2160))
            ...
            #pts2 = np.float32([[0, 0], [0, 2160], [3840, 0], [3840,2160]]) 

            matrix = cv2.getPerspectiveTransform(pts1, pts2) 
            inv_matrix = cv2.getPerspectiveTransform(pts2, pts1)
            transformed_frame = cv2.warpPerspective(frame, matrix, (640,480))
            #transformed_frame = cv2.warpPerspective(frame, matrix, (3840,2160))
            ...
            #cv2.rectangle(lane1_mask, (left_base-25, 0), (middle_base+25, 2160), (0, 255, 0), -1)
            #cv2.rectangle(lane2_mask, (middle_base-25, 0), (right_base+25, 2160), (255, 0, 0), -1)
            #3840-2160 6 -4.5
            ...
            #lane1_original = cv2.warpPerspective(lane1_mask, inv_matrix, (3840, 2160))
            #lane2_original = cv2.warpPerspective(lane2_mask, inv_matrix, (3840, 2160))
            #print("Frame shape:", frame.shape)
            #print("Lane1_original shape:", lane1_original.shape)
            ...
            lane_re = cv2.resize(lanes_original, (640, 480))
            #cv2.imshow("Lanes", lane_re)
    ...
    # Asegúrate de que las máscaras de los carriles estén en escala de grises
    with lock:
        lane1_mask_gray = cv2.cvtColor(lane1_mask, cv2.COLOR_BGR2GRAY)
        lane2_mask_gray = cv2.cvtColor(lane2_mask, cv2.COLOR_BGR2GRAY)

    #frame_queue.put(final_frame)
    vehicle_lanes = {}
    
    for track_id, position in vehicle_positions2.items():
        xmin, ymin, xmax, ymax = position  # Desempaquetar las coordenadas
    ...
    # Opcional: Imprimir la asociación de track_id con carril
    for vid, lane in vehicle_lanes.items():
        print(f"Track ID: {vid} está en el carril: {lane}")
    print("---------------")
